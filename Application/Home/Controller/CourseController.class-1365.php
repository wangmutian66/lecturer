...
class CourseController extends Controller {
    ...
    //     print_r($id);die;
    //     [0] => in
    // [1] => 143,144,149,150,152,157,160,161,163,164,165
    ...
    public function searove(){
		...
		$course = M('course as c')
				...
				->field('c.id as c_id,c.uid as c_uid,c.starttime as c_starttime,c.endtime as c_endtime,c.classname as c_clasname,c.place as c_place,u.name as u_name,e.Qyname as e_Qyname,c.conduct as conduct')
				->where($map)
				->limit($first,$limit)
				->select();
		// echo '<pre>123';
    ...
    public function page2(){
    	$uid = session("uid");
        $uid=getcompanyId();
		$information = M('information as f')->where(array('f.uid'=>$uid))->count();
	...
	public function edit(){
        ...
        if(I('post.typey') == 1){
			$information = M('information as f')->where(array('f.uid'=>$uid))->where($search)->limit($first,$limit)->order('f.id desc')->select();
		}else{
