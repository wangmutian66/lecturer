		...
		$.extend({
				"easypage":function(options){
					options = $.extend({//参数设置
						...
						everycount:"10",//每页显示多少个
						navigatecount:"5"//导航按钮一次显示多少个
						},
						options);
					var currentpage = 0;//当前页	
					var contentspages = $("#data ."+options.contentclass);//要显示的内容
					var contentcount = contentspages.length;//得到内容的个数
					var pagecount = Math.ceil(contentcount/options.everycount);//计算出页数
					//拼接导航按钮 <div id='pagefirst' class='pagefirst'><a href='javascript:void(0)'>First</a></div>
					...
					//拼接导航按钮 <div id='pagelast' class='pagelast'><a href='javascript:void(0)'>List</a></div>
		...
		$.extend({
				"easypage1":function(options){
					options = $.extend({//参数设置
						...
						options);
					var currentpage = 0;//当前页	
					var contentspages = $("#contentspages ."+options.contentclass);//要显示的内容
					var contentcount = contentspages.length;//得到内容的个数
					var pagecount = Math.ceil(contentcount/options.everycount);//计算出页数
					//拼接导航按钮 <div id='pagefirst' class='pagefirst'><a href='javascript:void(0)'>First</a></div>
					var navigatehtml = "<div id='pagepre' class='pagepre'><a href='javascript:void(0)'> < </a></div>";
