<?php
	...
	class QiniuStorage {
	    ...
	    public function putFile($fileName='',$mime = 'application/octet-stream', $checkCrc = false){

	    	$filePath = "./Public/Uploads/private/".$filename;
			...
			$upToken = $this->UploadToken($this->sk, $this->ak);
	        ...
	        if ($size <= 4194304) {
	            ...
	            return $this->put($upToken,$fileName,$data,$mime);
	        }

	        $res = $this->fileUpload($upToken,$fileName,$filePath,$size,$mime);
	    ...
	    // form上传
	    private function put( $upToken,$key,$data,$mime){
	        ...
	        $response = $this->multipartPost($this->QINIU_UP_HOST, $fields, 'file', $fname, $data, $mime);
	    ...
	    // 结合form上传
	    private function multipartPost($url,$fields,$name, $fileName,$fileBody, $mimeType = null) {
	        $data = array();
	        $mimeBoundary = md5(microtime());
